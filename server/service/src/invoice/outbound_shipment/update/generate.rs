...
fn should_update_batches_total_number_of_packs(
    ...
    // Outbound Shipment status order New, Allocated, Picked, Shipped
    match (&invoice.status, status) {
        (InvoiceRowStatus::New, Some(UpdateOutboundShipmentStatus::Picked)) => true,
        (InvoiceRowStatus::New, Some(UpdateOutboundShipmentStatus::Shipped)) => true,
        (InvoiceRowStatus::Allocated, Some(UpdateOutboundShipmentStatus::Picked)) => true,
        (InvoiceRowStatus::Allocated, Some(UpdateOutboundShipmentStatus::Shipped)) => true,
        _ => false,
    }
}
